...
export function checkNavidromeDependencies(): NavidromeDependencyStatus {
  ...
  const missingDependencies = [...(!dockerAvailable ? ["Docker"] : []), ...(!ffmpegAvailable ? ["ffmpeg"] : [])];
...
function logAlbumCompletion(album: AlbumFixture, albumStartedAt: number, logPerAlbum: boolean): void {
  if (!logPerAlbum) {
    return;
  }
...
async function createAlbumArtworkTemplate(rootDir: string): Promise<Buffer> {
  ...
  runFfmpeg(["-loglevel", "error", "-y", "-f", "lavfi", "-i", "color=c=0xc7673c:s=1200x1200", "-frames:v", "1", artworkPath], {
    log: false,
  });
...
async function fetchSubsonicAlbumList(connection: NavidromeConnection, size: number, offset = 0): Promise<unknown[] | unknown | undefined> {
...
export async function createNavidromeAdmin(baseUrl: string, timeoutMs = 60_000): Promise<void> {
  ...
  throw new Error(`Failed to create admin user within ${timeoutMs}ms${lastError ? `: ${lastError}` : ""}`);
}

export async function waitForNavidromeScan(connection: NavidromeConnection, expectedAlbumCount = 1, timeoutMs = 15_000): Promise<void> {
  const startedAt = Date.now();
