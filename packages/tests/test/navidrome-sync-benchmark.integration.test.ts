...
import { albumsTable, songsTable, SyncManager } from "@muswag/shared";
import {
...
async function withProfiledFetch<T>(profiler: FetchProfiler, run: () => Promise<T>): Promise<T> {
...
describeBenchmark("navidrome sync benchmark", () => {
  it("syncs a 10k song library and prints the sync duration", async () => {
    ...
    await withNavidromeLibrary(
      ...
      async (connection) => {
        ...
        try {
          ...
          const syncedAlbums = await benchmarkDb.db
