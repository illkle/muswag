...
import { albumWithSongsFixture, songFixture } from "#core/testing/sync-fixtures";
import {
  ...
  syncAlbumsInMemory,
} from "#core/testing/sync-testkit";
import { createInMemoryDb } from "#core/testing/database";
import { createCoverManager, createInitialSyncProgress, sync, syncAlbums } from "#core";
...
describe("syncAlbums", () => {
  ...
  it("paginates album list requests", async () => {
    ...
    const pages: AlbumID3[][] = [albums.slice(0, 500).map(({ song: _song, ...album }) => album), albums.slice(500).map(({ song: _song, ...album }) => album)];
  ...
  it("requires login before syncManager sync", async () => {
    ...
    await expect(sync(db, createCoverManager({ db, store: createMemoryCoverArtStore() }))).rejects.toThrow("login() must be called before sync()");
  });
