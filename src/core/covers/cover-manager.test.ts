...
import { createCoverManager, type CoverArtStore } from "#core";
import { albumWithSongsFixture } from "#core/testing/sync-fixtures";
...
describe("cover manager", () => {
  ...
  it("repairs a known-bad path and removes it after the replacement is persisted", async () => {
    ...
    const result = await createCoverManager({ db, store }).repair({ type: "album", id: "missing", coverArtId: "cover-missing" }, "/covers/missing.jpg");
  ...
  it("ignores a stale image failure after the database points at a newer cover", async () => {
    ...
    const result = await createCoverManager({ db, store }).repair({ type: "album", id: "fresh", coverArtId: "cover-fresh" }, "/covers/stale.jpg");
