...
import type { AlbumID3, AlbumWithSongsID3, Child, GetAlbumArgs, GetAlbumList2Args, GetIndexesArgs, IndexArtist } from "#subsonic-api";
import { type Album, createCoverArtStore, type CoverArtStore, type MuswagDb, syncAlbums } from "#core";
import { createNodeCoverArtFileSystem } from "#core/sync-node";
import { createInMemoryDb } from "./database.js";
...
export async function syncQuickInMemory(params: Parameters<typeof syncAlbumsInMemory>[0]) {
  ...
  const fakeApi = createFakeSubsonicApi(params.pages ?? [params.albums.map(toAlbumListItem)], new Map(params.albums.map((album) => [album.id, album])), {
