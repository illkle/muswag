import type { AlbumID3, AlbumWithSongsID3, ArtistID3, Child, Contributor, ReplayGain } from "#subsonic-api";
...
export function albumWithSongsFixture(overrides: Partial<AlbumWithSongsID3> & { song?: Child[] } = {}): AlbumWithSongsID3 {
  const albumId = overrides.id ?? id("album");
  const name = overrides.name ?? "Test Album";
  const artist = overrides.artist ?? "Test Artist";
  const songs = overrides.song ?? [
    songFixture({
      id: `${albumId}-song-1`,
      album: name,
      albumId,
