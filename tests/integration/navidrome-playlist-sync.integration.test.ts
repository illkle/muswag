...
import { addPlaylistEntry, createPlaylist, createPlaylistSyncManager, deletePlaylist, login, renamePlaylist } from "#core";
...
describeIfReady("Navidrome playlist sync", () => {
  it("round-trips offline writes, remote writes, ordering, duplicates, and deletion", async () => {
    ...
    try {
      ...
      expect((await api.getPlaylist({ id: created!.id })).playlist.entry?.map(({ id }) => id)).toEqual([songs[0]!.id, songs[1]!.id, songs[0]!.id]);
