...
import type { MuswagDb, SyncRecord } from "#core";
import { createCoverManager, login, sync } from "#core";
import { librarySetA, librarySetB, type AlbumFixture } from "./fixtures/library-sets.js";
import { assertNoDanglingRelations, countSongsInLibrary, coverArtStoreFor, expectSongMetadata, expectSyncedCounts, readFullState } from "#core/testing/sync-testkit";
import { checkNavidromeDependencies, createInMemoryDb, createNavidromeTestConnection, createTempCoverArtDir, type NavidromeTestConnection, type TempDir } from "./navidrome-testkit.js";
...
async function withSyncedNavidromeLibrary<T>(
  albums: AlbumFixture[],
  run: (ctx: { db: MuswagDb; connection: NavidromeTestConnection; coverArt: TempDir; syncOnce: () => Promise<SyncRecord>; readState: () => ReturnType<typeof readFullState> }) => Promise<T>,
): Promise<T> {
