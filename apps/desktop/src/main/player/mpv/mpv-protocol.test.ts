...
describe("mpv protocol", () => {
  ...
  it("parses responses and observed events", () => {
    ...
    expect(parseMpvMessage('{"event":"property-change","name":"time-pos","data":null}')).toEqual({ event: { positionSeconds: null, type: "time-pos-change" }, kind: "event" });
    ...
    expect(parseMpvMessage('{"event":"start-file","playlist_entry_id":7}')).toEqual({ event: { playlistEntryId: 7, type: "start-file" }, kind: "event" });
    expect(parseMpvMessage('{"event":"start-file"}')).toEqual({ event: { playlistEntryId: null, type: "start-file" }, kind: "event" });
    expect(parseMpvMessage('{"event":"end-file","reason":"error","playlist_entry_id":7,"file_error":"loading failed"}')).toEqual({
      event: { fileError: "loading failed", playlistEntryId: 7, reason: "error", type: "end-file" },
      kind: "event",
    });
    expect(parseMpvMessage('{"event":"end-file","reason":"eof"}')).toEqual({
      event: { fileError: null, playlistEntryId: null, reason: "eof", type: "end-file" },
      kind: "event",
    });
  });
