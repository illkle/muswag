...
function RouteComponent() {
  ...
  const appearsOnQuery = useLiveQuery(
    (q) =>
      q
        ...
        .innerJoin({ song: db.songs }, ({ album, song }) =>
          eq(album.id, song.albumId),
        )
        .fn.where(({ song }) =>
  ...
  const matchingSongQuery = useLiveQuery(
    (q) =>
      q
        .from({ song: db.songs })
        .fn.where(({ song }) =>
          getArtistCredits(song).some((artist) => artist.id === artistId),
        )
        .findOne(),
    [artistId],
  );
  ...
  const songCredit = matchingSongQuery.data
    ? getArtistCredits(matchingSongQuery.data).find(
        (credit) => credit.id === artistId,
      )
    : undefined;
  const artistName =
  ...
  if (
    ...
    appearsOnQuery.isLoading ||
    matchingSongQuery.isLoading
  ) {
    return (
  ...
  if (
    artistQuery.isError ||
    albumsQuery.isError ||
    appearsOnQuery.isError ||
    matchingSongQuery.isError
  ) {
    return (
      <section className="flex h-full w-full flex-col">
        <div className="m-6">
          <Alert variant="destructive">
            ...
            <AlertDescription>
              The artist could not be read from the local database.
            </AlertDescription>
          </Alert>
        </div>
      </section>
  ...
  const artistMeta = formatMetaLine([
    albums.length > 0
      ...
      : null,
    appearsOn.length > 0
      ? `${appearsOn.length} appearance${appearsOn.length === 1 ? '' : 's'}`
      : null,
  ]);
        ...
        topContent={
          ...
          >
            <p className="text-sm text-muted-foreground">
              {artistMeta || 'No albums in the synced local library.'}
            </p>
          </DetailHeader>
        }
      />
