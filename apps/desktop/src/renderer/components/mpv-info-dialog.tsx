import { AlertCircle, CheckCircle2, CircleX, Download, FolderOpen, Info, Loader2, RefreshCw } from "lucide-react";
...
const mpvStatusLabels: Record<MpvState["status"], string> = {
  checking: "Checking",
  ready: "Available",
  ...
  return (
    <>
      <SidebarMenuButton tooltip="MPV information" onClick={() => setOpen(true)}>
        {playerError || mpvState.status !== "ready" ? <AlertCircle /> : <Info />}
        MPV information
      </SidebarMenuButton>
          ...
          <div className="space-y-4">
            ...
            {mpvState.status === "ready" ? (
              <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-sm">
                ...
                <dd className="break-all font-mono text-xs">{mpvState.binaryPath}</dd>
            ...
            {installState.status === "failed" ? <p className="break-words text-sm text-destructive">{installState.error}</p> : null}
